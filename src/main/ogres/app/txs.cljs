...
(defmethod
  ...
  [_ label]
  (fn [_ local]
    [{:db/key (:db/key (:local/camera local)) :camera/label label}]))
...
(defmethod
  ...
  (fn [_ local]
    [{:db/key       (:db/key (:local/camera local))
      :camera/point [(round x 1) (round y 1)]}]))
...
(defmethod
  ^{:arglists '([event] [event scale] [event scale x y])
  ...
  ([event scale]
   (fn [_ local]
     (let [[_ _ w h] (or (:bounds/self local) [0 0 0 0])]
       (transact event scale (/ w 2) (/ h 2)))))
  ([_ scale x y]
   (fn [_ local]
     (let [[cx cy] (or (:camera/point (:local/camera local)) [0 0])
           prev    (or (:camera/scale (:local/camera local)) 1)
           fx      (/ scale prev)
           dx      (/ (- (* x fx) x) scale)
           dy      (/ (- (* y fx) y) scale)]
       [{:db/key       (:db/key (:local/camera local))
         :camera/point [(round (+ cx dx) 1) (round (+ cy dy) 1)]
         :camera/scale scale}]))))

(defmethod
  ^{:arglists '([event delta x y])
  ...
  (fn [_ local]
    (transact :camera/zoom-change
              (-> (:local/camera local) (:camera/scale) (or 1) (js/Math.log)
...
(defmethod
  ...
  []
  (fn [_ local]
    (let [prev (or (:camera/scale (:local/camera local)) 1)
          next (reduce (fn [n s] (if (> s prev) (reduced s) n)) prev zoom-scales)]
      (transact :camera/zoom-change next))))
...
(defmethod
  ^{:doc "Resets the zoom value for the given camera to its default setting of
          100%."}
  transact :camera/zoom-reset
  []
  (transact :camera/zoom-change 1))
...
(defmethod
  ...
  [_ key]
  [{:db/ident :local :local/camera {:db/key key}}])
    ...
    (let [select-w [:db/key {:camera/scene [:db/key]}]
      ...
      (cond
        ...
        (into [[:db/retractEntity [:db/key scene]]
               ...
               [:db/add [:db/ident :local] :local/camera -1]
               [:db/add [:db/ident :local] :local/cameras -1]]
              (comp cat cat)
...
(defmethod
  ...
  [_ checksum]
  (fn [_ local]
    (let [scene (:camera/scene (:local/camera local))]
      [{:db/key (:db/key scene) :scene/image {:image/checksum checksum}}])))
...
(defmethod
  ...
  (fn [_ local]
    (let [scene (:camera/scene (:local/camera local))]
      [{:db/key (:db/key scene) :scene/grid-size size}])))
...
(defmethod
  ...
  (fn [_ local]
    (let [camera (:local/camera local)
          scene  (:camera/scene camera)]
      [{:db/key (:db/key camera) :camera/draw-mode :select}
       [:db/retract [:db/key (:db/key scene)] :scene/grid-origin]])))
...
(defmethod
  ...
  (fn [_ local]
    (let [scene (:camera/scene (:local/camera local))]
      [[:db/retract [:db/key (:db/key scene)] :scene/grid-size]])))
...
(defmethod
  ...
  (fn [_ local]
    (let [scene (:camera/scene (:local/camera local))]
      [{:db/key (:db/key scene) :scene/show-grid value}])))
...
(defmethod
  ...
  (fn [_ local]
    (let [scene (:camera/scene (:local/camera local))]
      [{:db/key (:db/key scene) :scene/dark-mode enabled}])))
...
(defmethod
  ...
  (fn [_ local]
    (let [scene (:camera/scene (:local/camera local))]
      [{:db/key (:db/key scene) :scene/lighting value}])))
...
(defmethod
  ...
  (fn [_ local]
    (let [scene (:camera/scene (:local/camera local))]
      [{:db/key (:db/key scene) :scene/timeofday value}])))
...
(defmethod transact :element/select
  [_ key replace?]
  (fn [data local]
    (let [camera (:db/key (:local/camera local))
          lookup [:db/key key]
...
(defmethod transact :token/create
  ...
  (fn [_ local]
    (let [camera (:local/camera local)
      ...
      [{:db/id -1
        :db/key (squuid)
        :token/point [(round x 1) (round y 1)]
        :token/image {:image/checksum checksum}}
       {:db/key (:db/key camera)
        :camera/selected  -1
        :camera/draw-mode :select}
       {:db/key (:db/key scene) :scene/tokens -1}])))

(defmethod transact :token/remove
  [_ keys]
  (fn [data local]
    (let [scen (:db/key (:camera/scene (:local/camera local)))
          keys (set keys)
          cnvs (ds/entity data [:db/key scen])
...
(defmethod transact :shape/translate
  ...
  (fn [data]
    (let [result (ds/pull data [:shape/vecs] [:db/key key])
          ...
          y (round y 1)]
      [{:db/key key
        :shape/vecs (into [x y] (trans-xf (- x ax) (- y ay)) vecs)}])))
...
(defmethod transact :share/switch
  ([event]
   (fn [_ local]
     (transact event (not (:local/paused? local)))))
  ([_ paused?]
   (fn [_ local]
     [{:db/key (:db/key local) :local/paused? paused?}])))

(defmethod transact :bounds/change
  [_ w-type bounds]
  (fn [_ local]
    [[:db/add -1 :db/key (:db/key local)]
...
(defmethod transact :selection/from-rect
  ...
  (fn [data local]
    (let [camera (:local/camera local)
          scene  (:camera/scene camera)
          select [{:scene/tokens [:db/key :token/point [:token/flags :default #{}]]}]
          local  (ds/pull data [:local/type] [:db/key local])
          result (ds/pull data select [:db/key (:db/key scene)])
...
(defmethod transact :selection/remove
  ...
  (fn [data local]
    (let [camera (:db/key (:local/camera local))
          select [{:camera/selected [:db/key :scene/_tokens :scene/_shapes]}]
          result (ds/pull data select [:db/key camera])
          groups (->> (:camera/selected result)
                      (group-by (fn [x] (cond (:scene/_tokens x) :token (:scene/_shapes x) :shape)))
                      (first))]
      (case (key groups)
        :token (transact :token/remove (map :db/key (val groups)))
        :shape (transact :shape/remove (map :db/key (val groups)))))))

(defmethod transact :initiative/toggle
  [_ keys adding?]
  (fn [data local]
    (let [scene  (:db/key (:camera/scene (:local/camera local)))
...
(defmethod transact :initiative/roll-all
  ...
  (fn [data local]
    (let [scene  (:db/key (:camera/scene (:local/camera local)))
          select [{:scene/initiative [:db/key :token/flags :initiative/roll]}]
...
(defmethod transact :initiative/reset
  ...
  (fn [_ local]
    (let [scene (:camera/scene (:local/camera local))
          scene-key (:db/key scene)]
      (->> (for [[idx token] (sequence (indexed 2) (:scene/initiative scene))
                 :let [{key :db/key} token]]
             [[:db/add idx :db/key key]
              [:db/retract [:db/key key] :initiative/roll]])
           (into [[:db/add -1 :db/key scene-key]
                  [:db/retract [:db/key scene-key] :initiative/turn]
                  [:db/retract [:db/key scene-key] :initiative/turns]
                  [:db/retract [:db/key scene-key] :initiative/rounds]] cat)))))
...
(defmethod transact :initiative/leave
  ...
  (fn [_ local]
    (let [scene  (:camera/scene (:local/camera local))
      ...
      (apply concat
             [[:db/add -1 :db/key scene-key]
              [:db/retract [:db/key scene-key] :scene/initiative]
              [:db/retract [:db/key scene-key] :initiative/turn]
              [:db/retract [:db/key scene-key] :initiative/turns]
              [:db/retract [:db/key scene-key] :initiative/rounds]]
...
(defmethod
  ...
  (fn [_ local]
    (let [scene (:camera/scene (:local/camera local))]
      [{:db/id -1 :db/key (:db/key scene) :mask/filled? true}])))
...
(defmethod
  ...
  (fn [_ local]
    (let [scene (:camera/scene (:local/camera local))]
      [{:db/id -1 :db/key (:db/key scene) :mask/filled? false}])))
  ...
  ([_ cut?]
   (fn [data local]
     (let [attrs  [:token/label :token/flags :token/light :token/size :aura/radius :token/image :token/point]
           ...
           result (ds/pull data select [:db/key (:db/key (:local/camera local))])
...
(defmethod
  ...
  (fn [data local]
    (let [camera (:db/key (:local/camera local))
          ...
          result (ds/pull data clipboard-paste-select [:db/ident :root])
